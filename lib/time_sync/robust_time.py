...
class TimeManager:
    ...
    def __init__(self, state_manager, wifi_manager, timezone_offset=1):
        ...
        # NTP configuration - using IP addresses to avoid DNS issues
        self.ntp_servers = [
            "129.6.15.28",  # time.nist.gov
            "132.163.97.1",  # time-a.nist.gov
            "216.229.0.179",  # tick.ucla.edu
    ...
    def _sync_time_now(self, timeout=None):
        ...
        FIXED: Handle CircuitPython ESP32 Error 32 gracefully
        """
        if timeout is None:
        ...
        try:
            ...
            # Create NTP client with error handling for ESP32 socket bugs
            ntp_client = adafruit_ntp.NTP(
                socket_pool, server=server, tz_offset=self.timezone_offset
            )

            # Attempt sync with timeout and Error 32 handling
            ...
            while time.monotonic() - sync_start < timeout:
                try:
                    ...
                    # Handle Error 32 specifically
                    if str(e) == "32":
                        print(
                            f"     ⚠️ CircuitPython Error 32 (socket bug) - trying next server"
                        )
                        self._rotate_ntp_server()
                        raise Exception("Error 32 - CircuitPython socket bug")
                    else:
                        raise e
    ...
    def update(self):
        ...
        if self.wifi_manager.is_connected() and self.should_sync_time():
    ...
    def get_status(self):
        ...
        current_time = time.monotonic()
        time_since_sync = (
            current_time - self.last_sync_time if self.last_sync_time else None
        )
        ...
        if self.last_sync_time:
            if self.last_sync_success:
                ...
                next_sync_in = self.failed_sync_retry - (
                    current_time - self.last_sync_time
                )
